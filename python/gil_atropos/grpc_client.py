...
This module wraps the generated stubs (``session_pb2_grpc``,
``run_pb2_grpc``, ``interview_pb2_grpc``, ``event_pb2_grpc``) with an
ergonomic Python API. The stubs themselves are produced by
``gil_atropos.compile_protos``; until that runs, importing this module
still works -- the stub import is deferred until first use so that
``import gil_atropos`` succeeds in environments that haven't compiled
protos yet (e.g., during unit tests with mocks).
...
# rather than ImportError on bare module load.
try:
...
def _load_stubs():
    ...
    from gil_atropos import proto as _proto_pkg  # noqa: F401
    ...
    try:
        ...
        raise RuntimeError(
            "gil_atropos gRPC stubs are not compiled. Run:\n"
...
class GilGrpcClient:
    ...
    def __init__(
        ...
        *,
        ...
        # Defer stub binding until we have a channel and stubs in hand.
    ...
    def freeze_spec(self, session_id: str, spec_dict: Mapping[str, Any]) -> dict[str, str]:
        ...
        gil's normal flow runs the interview agent until ``Confirm`` is
        called. For RL rollouts we don't want a chat back-and-forth, so we
        emulate the "scripted interview" path:

        1. Send ``StartInterview`` with the goal one-liner as ``first_input``.
        2. Drain the resulting event stream up to the first agent question
           (or stage transition past Sensing).
        3. Immediately call ``Confirm`` to freeze whatever spec the agent
           extracted, falling back to the user-supplied ``spec_dict`` if the
           server supports a "force-spec" path.

        Until gild grows a true ``FreezeSpec(spec)`` RPC, this best-effort
        path is what's available. For training purposes the verifier in
        ``spec_dict`` is what really matters; ``spec_dict`` is also stashed
        on the client for later reward computation.
        """
        interview_pb2 = self._stubs["interview_pb2"]

        # Stash for the env to consult later (we don't get to enforce these
        # fields server-side without a richer RPC).
        self._last_spec_hint: dict[str, Any] = dict(spec_dict)

        first_input = (
            spec_dict.get("goal", {}).get("one_liner")
            or spec_dict.get("goal_hint")
            or "Coding task (auto-generated)"
        )
        ...
        # Drain just enough events to get past Sensing without blocking forever.
        # If the server emits an error we propagate it; otherwise we move on
        # to Confirm even if the agent would have asked questions.
        try:
            for ev in self.interview_stub.Start(
                ...
                if kind == "stage" and ev.stage.to.lower() in {"conversation", "draft", "frozen"}:
                    break
                if kind == "agent_turn":
                    # First agent question received -- enough signal to confirm.
                    break
        except Exception:
            # If Start fails (e.g. the session is already past interview), fall
    ...
    def stream_events(self, session_id: str) -> Iterator[dict[str, Any]]:
        ...
        Parses the JSON-encoded ``data_json`` payload to a dict for
        convenience. The generator terminates when the server closes the
        stream (run finished) or the channel errors out.
        """
        run_pb2 = self._stubs["run_pb2"]
        ...
        try:
            ...
            # grpc.RpcError on stream close is normal-ish; surface as final
            # synthetic event so callers can drain the loop cleanly.
