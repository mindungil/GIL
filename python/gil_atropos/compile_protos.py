...
Usage::
    ...
    python -m gil_atropos.compile_protos                       # default paths
    python -m gil_atropos.compile_protos --proto-root /path/to/gil/proto

Writes ``*_pb2.py`` and ``*_pb2_grpc.py`` into ``gil_atropos/proto/`` (the
``proto`` subpackage of this distribution). Generated files are not
committed -- ``.gitignore`` excludes them. Run after a ``pip install -e .``
or whenever the .proto files change.
...
def _default_proto_root() -> Path:
    ...
    Works in two layouts:
      * In-tree dev: ``<gil-repo>/python/gil_atropos/`` -> ``../../proto``
      * Installed wheel: user must pass ``--proto-root`` explicitly.
    """
    here = Path(__file__).resolve().parent  # .../gil_atropos
    candidate = here.parent.parent / "proto"  # .../gil/proto
    return candidate
...
def compile_protos(proto_root: Path, out_dir: Path) -> int:
    ...
    # Build the protoc argv. We include googleapis if we can find it (gil's
    # protos import google/api/annotations.proto for gRPC-gateway HTTP rules).
    argv = [
        "protoc",
    ...
    # Try to add common include paths for googleapis. This keeps users from
    # having to install a separate googleapis tarball when they only want the
...
def _googleapis_include_paths() -> list[Path]:
    """Look for googleapis .proto files bundled with grpc_tools or the system."""
    paths: list[Path] = []
    # grpc_tools ships with google/protobuf/*.proto but not googleapis.
    # Check a few well-known locations.
    candidates = [
        Path("/usr/include"),
    ...
    # If grpc_tools _proto include dir exists, add it too -- it has
    # google/protobuf/*.proto which gil's protos transitively need.
    try:
